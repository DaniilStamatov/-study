...
namespace transport {
    ...
    void PrintRoute(std::string& line, Catalogue& catalogue, std::ostream& out) {
        ...
        if (catalogue.FindBus(bus_number)) {
            out << "Bus " << bus_number << ": " << catalogue.GetBusInfo(bus_number)->stop_count << " stops on route, "
                << catalogue.GetBusInfo(bus_number)->unique_stop_count << " unique stops, " << std::setprecision(6)
                << catalogue.GetBusInfo(bus_number)->route_length << " route length, "
                << catalogue.GetBusInfo(bus_number)->curvature << " curvature\n";
        }
        else {
