...
void foo() {
    int local_var = 20; // automatic storage duration
    static int static_local_var = 30; // static storage duration
...
public:
    ...
    T& operator*() const {
        if (!*this)
            throw std::logic_error("Ты не можешь так");
        return *ptr_;
        // Реализуйте самостоятельно
    }
...
public:
    ...
    explicit Octopus(int num_tentacles) {
        ...
        try {
            for (int i = 1; i <= num_tentacles; ++i) {
                ScopedPtr<Tentacle> ptr(new Tentacle(i));     // Может выбросить исключение bad_alloc
                tentacles_.GetItems().push_back(ptr.GetRawPtr());  // Может выбросить исключение bad_alloc
    ...
    Octopus(const Octopus& other): tentacles_(other.tentacles_) {
        /*for (Tentacle* t : other.tentacles_.GetItems()) {
            if (t != nullptr) {
                try {
                    ScopedPtr<Tentacle> ptr(new Tentacle(*t));
                ...
                tentacles_.GetItems().push_back(nullptr);
            }
    ...
    // Возвращает ссылку на добавленное щупальце
     Tentacle& AddTentacle() {
         ...
         tentacles_.GetItems().emplace_back(ptr.GetRawPtr());
         ptr.Release();
